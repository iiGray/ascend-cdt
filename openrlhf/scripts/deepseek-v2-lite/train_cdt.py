...
def train(args):
    ...
    model_name = os.path.basename(args.pretrain.rstrip('/'))

    if not os.path.exists(f"{args.save_dir}/dataset_files/train_{model_name}_{args.max_len}.pkl"):
        if dist.get_rank() == 0:
            os.makedirs(f"{args.save_dir}/dataset_files/", exist_ok = True)
            ...
            with open(f"{args.save_dir}/dataset_files/train_{model_name}_{args.max_len}.pkl", "wb") as f:
                pickle.dump(train_dataset, f)

            with open(f"{args.save_dir}/dataset_files/eval_{model_name}_{args.max_len}.pkl", "wb") as f:
                pickle.dump(eval_dataset, f)

        dist.barrier()


    train_dataset = pickle.load(open(f"{args.save_dir}/dataset_files/train_{model_name}_{args.max_len}.pkl","rb"))
    eval_dataset = pickle.load(open(f"{args.save_dir}/dataset_files/eval_{model_name}_{args.max_len}.pkl","rb"))
